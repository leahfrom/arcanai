...
export const runDirect = async (options: CliOptions): Promise<void> => {
  ...
  const cards = drawCards({spread: options.spread, allowReversed: options.allowReversed});
  ...
  console.log(options.question ? `Question: ${options.question}` : 'Question: General reading');
  console.log('');
  ...
  console.log('');
  console.log(`Reading via ${reading.provider}${reading.model ? ` / ${reading.model}` : ''}${reading.usedFallback ? ' / fallback' : ''}`);
  console.log('');
